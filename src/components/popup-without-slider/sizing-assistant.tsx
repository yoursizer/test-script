...
import { MobileMeasurementSlider } from "./mobile-sliders/MobileMeasurementSlider"
import { calculateBodyMeasurements } from "../bmi"
import { LicenseOverlay } from "../invalid-overlay/license-overlay"
...
export function SizingAssistant({ onClose, onSizeRecommended, onAddToBag, productId, brandId, clothingType, licenseStatus, licenseKey, onLicenseRetry, isChecking }: SizingAssistantProps) {
  console.log('🚀 SizingAssistant (popup-without-slider) rendered with:', { productId, brandId, clothingType, licenseKey });
  ...
  const [showLicenseOverlay, setShowLicenseOverlay] = useState(false);
  ...
  const startStepTracking = (stepId: string) => {
    analytics.startStepTracking(stepId);
    setStepStartTime(Date.now());
    setCurrentStepId(stepId);
  };

  const trackStepCompletion = async (stepId: string, stepName: string) => {
    await analytics.trackStepCompleted(licenseKey, stepName, stepId);
  };

  const trackMeasurementEntry = async (measurementType: string, stepId: string) => {
    await analytics.trackMeasurementEntered(licenseKey, measurementType, stepId);
  };

  const trackGenderSelection = async (selectedGender: 'male' | 'female') => {
    await analytics.trackGenderSelected(licenseKey, selectedGender);
  };

  const trackMeasurementGuideOpened = async (measurementType: 'chest' | 'waist' | 'hips') => {
    await analytics.trackMeasurementGuideOpened(licenseKey, measurementType);
  };

  const trackAddToCart = useCallback(async (size: string) => {
    console.log('🔍 trackAddToCart called in popup-without-slider:', { productId, licenseKey, size });
    if (productId) {
      console.log('✅ productId exists, calling analytics.trackAddToCart');
      await analytics.trackAddToCart(licenseKey, productId, size);
    } else {
      console.log('❌ productId is falsy, skipping analytics call');
    }
  }, [licenseKey, productId]);

  const trackPopupCompletion = useCallback(async () => {
    await analytics.trackPopupCompleted(licenseKey);
  }, [licenseKey]);

  const trackSizeRecommendation = useCallback(async (size: string) => {
    if (gender && !hasTrackedRecommendation) {
      await analytics.trackSizeRecommended(licenseKey, gender, size);
      setHasTrackedRecommendation(true);
    }
  }, [licenseKey, gender, hasTrackedRecommendation]);
  ...
  useEffect(() => {
    ...
    // Start tracking the first step
    startStepTracking('step_1_gender_selection');
  ...
  // Cache for BMI limits to prevent unnecessary console logs
  const bmiLimitsCache = useRef<{
    [key: string]: {
      height: number;
      weight: number;
      gender: string;
  ...
  // Get BMI-based dynamic limits for chest, waist, hips
  const getBMIBasedLimits = (measurementType: 'chest' | 'waist' | 'hips') => {
    if (!gender || !measurements.height || !measurements.weight) {
      return MEASUREMENT_LIMITS[measurementType];
    }
    ...
    // Create cache key
    const cacheKey = `${measurementType}_${height}_${weight}_${gender}`;
    const cached = bmiLimitsCache.current[cacheKey];
    
    // Return cached value if same inputs
    if (cached && 
    ...
    const bmiMeasurements = calculateBodyMeasurements(height, weight, gender);
    const baseValue = bmiMeasurements[measurementType];
    
    // Clamp the base value from BMI to be within the absolute min/max for this measurement type.
    // This prevents issues where an extreme BMI calculation could result in an invalid range (e.g. min > max).
    ...
    // Ensure the slider is always centered on the clampedBaseValue.
    // The slider's travel distance is determined by the shortest distance to an absolute limit, capped at 6.
    const upperDistance = MEASUREMENT_LIMITS[measurementType].max - clampedBaseValue;
    const lowerDistance = clampedBaseValue - MEASUREMENT_LIMITS[measurementType].min;
    const sliderRadius = Math.min(upperDistance, lowerDistance, 6);

    const limits = {
      min: clampedBaseValue - sliderRadius,
      max: clampedBaseValue + sliderRadius,
      default: clampedBaseValue
    };

    // Cache the result
    bmiLimitsCache.current[cacheKey] = {
      height,
      weight,
      gender,
      limits
    ...
    console.log(`🔧 BMI-based limits for ${measurementType} (NEW):`, {
      baseValue: clampedBaseValue,
      limits,
      bmiMeasurements: { height, weight, gender }
    });
  ...
  // Calculate BMI measurements ONLY when height/weight actually change
  useEffect(() => {
    // This effect now correctly handles updates after flags are reset.
    console.log('🔄 BMI useEffect triggered:', {
      gender,
    ...
    if (gender && measurements.height && measurements.weight) {
      // The key condition: if any of the measurements are NOT user-edited,
      // it means we should be applying BMI calculations. This is especially
      // true after height/weight change, because another effect will have
      // just reset these flags to `false`.
      if (!userEditedMeasurements.chest || !userEditedMeasurements.waist || !userEditedMeasurements.hips) {
        ...
        if (!isNaN(height) && !isNaN(weight)) {
          ...
          console.log('🧮 Applying BMI Calculations:', {
            input: { height, weight, gender },
            calculated: bmiMeasurements,
            willUpdate: {
              chest: !userEditedMeasurements.chest,
              waist: !userEditedMeasurements.waist,
              hips: !userEditedMeasurements.hips
            }
          });
          
          setMeasurements(prev => {
            // Only update the values that were not manually edited.
            const newMeasurements = {
              ...prev,
              chest: !userEditedMeasurements.chest ? bmiMeasurements.chest.toString() : prev.chest,
              waist: !userEditedMeasurements.waist ? bmiMeasurements.waist.toString() : prev.waist,
              hips: !userEditedMeasurements.hips ? bmiMeasurements.hips.toString() : prev.hips,
            };
         ...
         console.log('⏸️ BMI calculation skipped - all measurements were manually edited.');
      }
    }
  }, [gender, measurements.height, measurements.weight, userEditedMeasurements]); // Dependency on flags is crucial

  // Reset user edited flags when height/weight changes to allow BMI override
  useEffect(() => {
    // This effect runs when height or weight changes. It resets the flags
    // that prevent BMI calculations from overwriting user's manual inputs
    // for chest, waist, and hips. This ensures that after adjusting height
    // or weight, the body measurements are always recalculated.
    setUserEditedMeasurements({ chest: false, waist: false, hips: false });
  }, [measurements.height, measurements.weight]);

  const updateModelDimensions = useCallback((model: THREE.Group, measurements: any) => {
    ...
    let bmiBaseChest = gender === 'male' ? 100 : 90;
    let bmiBaseWaist = gender === 'male' ? 85 : 75;
    let bmiBaseHips = gender === 'male' ? 95 : 100;

    if (gender && !isNaN(height) && !isNaN(weight)) {
      const bmiMeasurements = calculateBodyMeasurements(height, weight, gender);
      bmiBaseChest = bmiMeasurements.chest;
      bmiBaseWaist = bmiMeasurements.waist;
      bmiBaseHips = bmiMeasurements.hips;
    }

    const chestLimits = getBMIBasedLimits('chest');
    const waistLimits = getBMIBasedLimits('waist');
    const hipsLimits = getBMIBasedLimits('hips');

    const clampedChest = Math.min(Math.max(chest, chestLimits.min), chestLimits.max);
    const clampedWaist = Math.min(Math.max(waist, waistLimits.min), waistLimits.max);
    const clampedHips = Math.min(Math.max(hips, hipsLimits.min), hipsLimits.max);
    ...
    const normalizedWeight = (clampedWeight - baseWeight) / (WEIGHT_LIMITS.max - baseWeight);
    ...
    const chestDiff = (clampedChest - bmiBaseChest) / 6;
    const waistDiff = (clampedWaist - bmiBaseWaist) / 6;
    const hipsDiff = (clampedHips - bmiBaseHips) / 6;

    const chestValue = Math.min(Math.max(chestDiff, -1.0), 1.0);
    const waistValue = Math.min(Math.max(waistDiff, -1.0), 1.0);
    const hipsValue = Math.min(Math.max(hipsDiff, -1.0), 1.0);

    // Determine which morphs to apply based on the current step
    const applyHeight = (isMobile && step >= 2) || (!isMobile && step >= 2);
    const applyWeight = (isMobile && step >= 3) || (!isMobile && step >= 2);
    const applyChest = (isMobile && step >= 4) || (!isMobile && step >= 3);
    const applyWaist = (isMobile && step >= 5) || (!isMobile && step >= 3);
    const applyHips = (isMobile && step >= 6) || (!isMobile && step >= 3);
    ...
    morphTargets.forEach((morphData, morphName) => {
      morphData.meshes.forEach(({ mesh, index }) => {
        ...
        switch (morphName) {
          case 'height_200':
            targetValue = applyHeight ? heightValue : 0;
            break;
          case 'male_overweight':
            targetValue = applyWeight && gender === 'male' ? clampedWeightValue : 0;
            break;
          case 'male_skinny':
            targetValue = applyWeight && gender === 'male' ? -clampedWeightValue : 0;
            break;
          case 'female_overweight':
            targetValue = applyWeight && gender === 'female' ? clampedWeightValue : 0;
            break;
          case 'Chest Width':
            targetValue = applyChest ? chestValue : 0;
            break;
          case 'Shoulder Width':
            targetValue = applyChest ? chestValue * 0.8 : 0;
            break;
          case 'Waist Thickness':
            targetValue = applyWaist ? waistValue : 0;
            break;
          case 'Hips Size':
            targetValue = applyHips ? hipsValue : 0;
            break;
          case 'Belly Size':
            targetValue = applyWaist ? waistValue * 0.8 : 0;
            break;
          case 'Chest Height':
            targetValue = applyHeight ? heightValue * 0.5 : 0;
            break;
          case 'Hips Height':
            targetValue = applyHeight ? heightValue * 0.5 : 0;
            break;
          case 'Thigh Thickness':
            targetValue = applyHips ? hipsValue * 0.7 : 0;
            break;
          case 'Thigh Length':
            targetValue = applyHeight ? heightValue * 0.3 : 0;
            break;
        }
  ...
  }, [gender, measurements, step, isMobile, getBMIBasedLimits, morphTargets]);
  ...
  }, [measurements, step, isMobile, updateModelDimensions])
  ...
  const getApiSizeRecommendation = async () => {
    ...
    // Extract product and brand information
    const getProductInfo = () => {
      try {
        ...
        if (window.location.pathname.includes('/products/')) {
          ...
          // Replace any special characters with regular ones for product_id
          productHandle = productHandle.replace(/İ/g, 'i').replace(/ı/g, 'i');
          
        }
        
        // Extract brand from domain or product handle
        const hostname = window.location.hostname;
        if (hostname.includes('kayra')) {
          brandName = 'kayra';
        } else if (hostname.includes('.')) {
          // Try to extract brand from subdomain or domain
          const domainParts = hostname.split('.');
          brandName = domainParts[0] === 'www' ? domainParts[1] : domainParts[0];
        }
        
        
        
        // Format product_id: brandname_productname-with-dashes
        const product_id = productHandle ? `${brandName}_${productHandle}` : `${brandName}_unknown-product`;
        
        
        return { brandName, product_id };
      } catch (error) {
        return { brandName: 'generic', product_id: 'generic_unknown-product' };
      }
    };

    const { brandName, product_id } = getProductInfo();
    ...
    const chestRaw = measurements.chest && !isNaN(parseFloat(measurements.chest))
      ? parseFloat(measurements.chest)
      : (gender === 'male' ? 100 : 90);
    
    const waistRaw = measurements.waist && !isNaN(parseFloat(measurements.waist))
      ? parseFloat(measurements.waist)
      : (gender === 'male' ? 85 : 75);
    
    const hipRaw = measurements.hips && !isNaN(parseFloat(measurements.hips))
      ? parseFloat(measurements.hips)
      : (gender === 'male' ? 95 : 100);

    // CRITICAL: Clamp all measurements to their absolute limits before sending to the API.
    // This is the final safeguard to prevent out-of-bounds values from being processed.
    const chest = Math.max(MEASUREMENT_LIMITS.chest.min, Math.min(MEASUREMENT_LIMITS.chest.max, chestRaw));
    const waist = Math.max(MEASUREMENT_LIMITS.waist.min, Math.min(MEASUREMENT_LIMITS.waist.max, waistRaw));
    const hip = Math.max(MEASUREMENT_LIMITS.hips.min, Math.min(MEASUREMENT_LIMITS.hips.max, hipRaw));

    console.log('📡 API Request measurements (clamped):', {
      chest, waist, hip, height, weight,
      userEdited: userEditedMeasurements,
      originalMeasurements: measurements,
      raw: { chestRaw, waistRaw, hipRaw }
    });
    ...
    const requestData = {
      ...
      measurements: {
        chest,
        waist,
        hip,  // Changed from 'hips' to 'hip'
        height,
        weight
      },
      fit_preference: fitPreference,
      clothing_type: clothingType || "", // Use prop or fallback to default
      use_ai: true,
      brand_id: brandId || brandName,  // Use prop or fallback to extracted brand
      product_id: productId || product_id,  // Use prop or fallback to extracted product_id
      license_key: licenseKey // Add license key for analytics tracking
    };
    ...
    if (step === 5) {
      if (useApiRecommendation) {
        ...
        if (apiResult) {
          ...
          await trackSizeRecommendation(apiResult.size);
          ...
          // Track size recommendation
          await trackSizeRecommendation(size);
          // Don't call onSizeRecommended here to prevent auto-closing
  ...
  const handleAddToBag = async () => {
    console.log('🛒 handleAddToBag called in popup-without-slider');
    ...
    console.log(`Add to Bag clicked with size: ${size}`);

    // Track add to cart
    console.log('📞 About to call trackAddToCart...');
    await trackAddToCart(size);
    console.log('📞 trackAddToCart completed');

    // Call the onAddToBag callback if provided
    if (onAddToBag) {
      onAddToBag(size);
    }
  ...
  const renderMeasurementInputs = (type: 'height' | 'weight' | 'chest' | 'waist' | 'hips') => {
    ...
    // Use BMI-based limits for chest, waist, hips
    const getMetricRange = (measurementType: typeof type) => {
      if (measurementType === 'height') {
        ...
        // For chest, waist, hips - use BMI-based ±6 range
        return getBMIBasedLimits(measurementType as 'chest' | 'waist' | 'hips');
      }
    };
    ...
    // Only override for desktop and chest/waist/hips
    if (!isMobile && (type === 'chest' || type === 'waist' || type === 'hips')) {
      let mainLabel = '';
      if (type === 'chest') {
        mainLabel = 'Chest';
      } else if (type === 'hips') {
      ...
      // Calculate min/max for slider using BMI-based limits
      const limits = getBMIBasedLimits(type as 'chest' | 'waist' | 'hips');
      const { min, max, default: bmiDefault } = limits;

      // Get current value from measurements - use BMI default if no value
      const currentValue = measurements[type] === '' || isNaN(parseFloat(measurements[type])) ? bmiDefault : parseFloat(measurements[type]);
      
      // Use continuous step instead of snapping
      const step = 0.5; // Continuous movement
      
      // When slider changes, update measurement and model
      const handleSliderChange = (v: number) => {
        const prevValue = parseFloat(measurements[type]) || bmiDefault;
        setMeasurements(prev => ({ ...prev, [type]: v.toFixed(1) }));
        
        // Log measurement change
        const change = v - bmiDefault;
        const changeFromPrev = v - prevValue;
        console.log(`📊 Desktop ${type} changed:`, {
          from: prevValue.toFixed(1),
          to: v.toFixed(1),
          bmiBase: bmiDefault.toFixed(1),
          changeFromBMI: change > 0 ? `+${change.toFixed(1)}` : change.toFixed(1),
          changeFromPrev: changeFromPrev > 0 ? `+${changeFromPrev.toFixed(1)}` : changeFromPrev.toFixed(1)
        });
        
        // Update morph targets for chest, waist, hips with 1x multiplier
        if (type === 'chest' || type === 'waist' || type === 'hips') {
          const bmiLimits = getBMIBasedLimits(type as 'chest' | 'waist' | 'hips');
          const bmiBase = bmiLimits.default;
          const diff = (v - bmiBase) / 6; // Normalize to -1 to 1 range
          const morphValue = Math.min(Math.max(diff, -1.0), 1.0); // 1x multiplier for morph
          
          if (type === 'chest') {
            handleMorphChange('Chest Width', morphValue);
          } else if (type === 'waist') {
            handleMorphChange('Waist Thickness', morphValue);
          } else if (type === 'hips') {
            handleMorphChange('Hips Size', morphValue);
          }
        }
      };
    ...
    const handleSliderChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      trackMeasurementEntry(type, `step_${step}_${type}`);
        ...
        <div className="space-y-2 md:space-y-3">
          ...
          <RadialWeightSlider
            ...
            onChange={(newValue) => {
              ...
              // Track measurement entry
              trackMeasurementEntry('weight', `step_${step}_weight`);
            }}
            useMetric={useMetric}
          />
        </div>
        ...
        {!isMobile && (
          <div className="flex items-center justify-between">
            ...
            {(type === 'chest' || type === 'waist' || type === 'hips') && (
              <button
                onClick={async () => {
                  setShowMeasurementGuide(type as any);
                  // Track measurement guide opened
                  await trackMeasurementGuideOpened(type as 'chest' | 'waist' | 'hips');
                }}
  ...
  useEffect(() => {
    ...
    if (previousStep !== 0 && previousStep !== currentStep) {
      const stepNames = {
        ...
        3: 'body_measurements',
        4: 'fit_preference',
        5: 'size_recommendation'
      };
      
      const previousStepName = stepNames[previousStep as keyof typeof stepNames] || `step_${previousStep}`;
      const currentStepName = stepNames[currentStep as keyof typeof stepNames] || `step_${currentStep}`;
      
      // Track completion of previous step
      trackStepCompletion(`step_${previousStep}_${previousStepName}`, previousStepName);
      
      // Start tracking new step
      startStepTracking(`step_${currentStep}_${currentStepName}`);
    }
                    ...
                    return (
                      <button
                        ...
                        onClick={async (e) => {
                          ...
                          await trackGenderSelection(newGender);
                        }}
              ...
              {isMobile ? (
                <>
                  ...
                  <RadialHeightSlider
                    ...
                    onChange={val => {
                      setMeasurements(prev => ({ ...prev, height: val.toString() }));
                      // Track measurement entry
                      trackMeasurementEntry('height', `step_${step}_height`);
                    }}
                    useMetric={useMetric}
                  />
              ...
              <div className="flex flex-col md:flex-row gap-3 md:gap-4">
                {[
                  ...
                  <button
                    key={type}
                    onClick={async () => {
                      setFitPreference(type as "slim" | "regular" | "relaxed");
                      // Track measurement entry for fit preference
                      await trackMeasurementEntry('fit_preference', `step_${step}_fit_preference`);
                    }}
                ...
                <div className="text-center">
                  ...
                  {apiSizeResult?.method === "ai" && (
                    <div className="flex items-center justify-center gap-1 mt-2 text-xs text-purple-600">
                      ...
                      <span>Your Sizer AI powered recommendation</span>
                    </div>
                  )}
                </div>
              )}
        ...
        return (
          <div className="">
            ...
            {isMobile ? (
              (() => {
                const chestLimits = getBMIBasedLimits('chest');
                const chestValue = measurements.chest && !isNaN(parseFloat(measurements.chest)) ? parseFloat(measurements.chest) : chestLimits.default;
                
                return (
                  <MobileMeasurementSlider
                    value={chestValue}
                    onChange={val => {
                      const prevValue = parseFloat(measurements.chest) || chestLimits.default;
                      setMeasurements(prev => ({ ...prev, chest: val.toString() }));
                      setUserEditedMeasurements(prev => ({ ...prev, chest: true }));
                      
                      // Log measurement change
                      const change = val - chestLimits.default;
                      const changeFromPrev = val - prevValue;
                      console.log(`📱 Mobile chest changed:`, {
                        from: prevValue.toFixed(1),
                        to: val.toFixed(1),
                        bmiBase: chestLimits.default.toFixed(1),
                        changeFromBMI: change > 0 ? `+${change.toFixed(1)}` : change.toFixed(1),
                        changeFromPrev: changeFromPrev > 0 ? `+${changeFromPrev.toFixed(1)}` : changeFromPrev.toFixed(1)
                      });
                      
                      // Calculate morph value with 1x multiplier
                      const bmiLimits = getBMIBasedLimits('chest');
                      const bmiBase = bmiLimits.default;
                      const diff = (val - bmiBase) / 6; // Normalize to -1 to 1 range
                      const morphValue = Math.min(Math.max(diff, -1.0), 1.0); // 1x multiplier for morph
                      handleMorphChange('Chest Width', morphValue);
                      
                      // Track measurement entry
                      trackMeasurementEntry('chest', `step_${step}_chest`);
                    }}
                    min={chestLimits.min}
                    max={chestLimits.max}
                    step={0.5}
        ...
        return (
          <div className="">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={async () => {
                    ...
                    // Track measurement guide opened
                    await trackMeasurementGuideOpened('waist');
                  }}
                  className="text-blue-600 hover:text-blue-700"
                >
                  <Info className="w-4 h-4" />
            ...
            {isMobile ? (
              (() => {
                const waistLimits = getBMIBasedLimits('waist');
                const waistValue = measurements.waist && !isNaN(parseFloat(measurements.waist)) ? parseFloat(measurements.waist) : waistLimits.default;
                
                return (
                  <MobileMeasurementSlider
                    value={waistValue}
                    onChange={val => {
                      const prevValue = parseFloat(measurements.waist) || waistLimits.default;
                      setMeasurements(prev => ({ ...prev, waist: val.toString() }));
                      setUserEditedMeasurements(prev => ({ ...prev, waist: true }));
                      
                      // Log measurement change
                      const change = val - waistLimits.default;
                      const changeFromPrev = val - prevValue;
                      console.log(`📱 Mobile waist changed:`, {
                        from: prevValue.toFixed(1),
                        to: val.toFixed(1),
                        bmiBase: waistLimits.default.toFixed(1),
                        changeFromBMI: change > 0 ? `+${change.toFixed(1)}` : change.toFixed(1),
                        changeFromPrev: changeFromPrev > 0 ? `+${changeFromPrev.toFixed(1)}` : changeFromPrev.toFixed(1)
                      });
                      
                      // Calculate morph value with 1x multiplier
                      const bmiLimits = getBMIBasedLimits('waist');
                      const bmiBase = bmiLimits.default;
                      const diff = (val - bmiBase) / 6; // Normalize to -1 to 1 range
                      const morphValue = Math.min(Math.max(diff, -1.0), 1.0); // 1x multiplier for morph
                      handleMorphChange('Waist Thickness', morphValue);
                      ...
                      // Track measurement entry
                      trackMeasurementEntry('waist', `step_${step}_waist`);
                    }}
                    min={waistLimits.min}
                    max={waistLimits.max}
                    step={0.5}
                    labels={["Very Slim", "Slim", "Normal", "Broad", "Very Broad"]}
                    label="Waist"
                  />
                );
              })()
        ...
        return (
          <div className="">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={async () => {
                    ...
                    // Track measurement guide opened
                    await trackMeasurementGuideOpened('hips');
                  }}
                  className="text-blue-600 hover:text-blue-700"
                >
                  <Info className="w-4 h-4" />
            ...
            {isMobile ? (
              (() => {
                const hipsLimits = getBMIBasedLimits('hips');
                const hipsValue = measurements.hips && !isNaN(parseFloat(measurements.hips)) ? parseFloat(measurements.hips) : hipsLimits.default;
                
                return (
                  <MobileMeasurementSlider
                    value={hipsValue}
                    onChange={val => {
                      const prevValue = parseFloat(measurements.hips) || hipsLimits.default;
                      setMeasurements(prev => ({ ...prev, hips: val.toString() }));
                      setUserEditedMeasurements(prev => ({ ...prev, hips: true }));
                      
                      // Log measurement change
                      const change = val - hipsLimits.default;
                      const changeFromPrev = val - prevValue;
                      console.log(`📱 Mobile hips changed:`, {
                        from: prevValue.toFixed(1),
                        to: val.toFixed(1),
                        bmiBase: hipsLimits.default.toFixed(1),
                        changeFromBMI: change > 0 ? `+${change.toFixed(1)}` : change.toFixed(1),
                        changeFromPrev: changeFromPrev > 0 ? `+${changeFromPrev.toFixed(1)}` : changeFromPrev.toFixed(1)
                      });
                      
                      // Calculate morph value with 1x multiplier
                      const bmiLimits = getBMIBasedLimits('hips');
                      const bmiBase = bmiLimits.default;
                      const diff = (val - bmiBase) / 6; // Normalize to -1 to 1 range
                      const morphValue = Math.min(Math.max(diff, -1.0), 1.0); // 1x multiplier for morph
                      handleMorphChange('Hips Size', morphValue);
                      ...
                      // Track measurement entry
                      trackMeasurementEntry('hips', `step_${step}_hips`);
                    }}
                    min={hipsLimits.min}
                    max={hipsLimits.max}
                    step={0.5}
  ...
  const handleActualClose = async () => {
    ...
    await trackPopupCompletion();
    setHasTrackedRecommendation(false);
    onClose();
  };

  const handleLicenseRetry = () => {
    setShowLicenseOverlay(false);
    if (onLicenseRetry) {
      onLicenseRetry();
    }
  };

  const handleLicenseOverlayClose = () => {
    setShowLicenseOverlay(false);
    onClose();
  };
  ...
  // Clear API result when measurements change (to trigger new calculation)
  useEffect(() => {
    if (apiSizeResult) {
      setApiSizeResult(null);
    }
  }, [measurements, gender, fitPreference]);
  ...
  }, [step, isMobile, useApiRecommendation, apiSizeResult, isApiLoading]);

  // Show license overlay when license is invalid
  useEffect(() => {
    if (licenseStatus && !licenseStatus.isValid) {
      setShowLicenseOverlay(true);
    }
  }, [licenseStatus]);
  ...
  return (
    ...
    >
                    ...
                    {step > 1 && (
                      <button
                        onClick={async () => {
                          ...
                          const currentStepName = stepNames[step as keyof typeof stepNames] || `step_${step}`;
                          await trackStepCompletion(`step_${step}_${currentStepName}`, currentStepName);
                        }}
      ...
      <LicenseOverlay
        ...
        onClose={handleLicenseOverlayClose}
        onRetry={handleLicenseRetry}
        isVisible={showLicenseOverlay}
