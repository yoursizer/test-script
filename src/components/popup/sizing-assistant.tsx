...
export function SizingAssistant({ onClose, onSizeRecommended, onAddToBag, productId, brandId, clothingType, licenseStatus, licenseKey, onLicenseRetry, isChecking }: SizingAssistantProps) {
  console.log('🚀 SizingAssistant (popup) rendered with:', { productId, brandId, clothingType, licenseKey });
  ...
  const startStepTracking = useCallback((stepId: string) => {
    analytics.startStepTracking(stepId);
  }, []);

  // Track step completion with time spent
  const trackStepCompletion = useCallback(async (stepId: string, stepName: string) => {
    await analytics.trackStepCompletion(stepId, {
  ...
  const trackMeasurementEntry = useCallback(async (measurementType: string, stepId: string) => {
    await analytics.trackMeasurementEntered(licenseKey, measurementType, stepId);
  }, [licenseKey]);

  // Track gender selection
  const trackGenderSelection = useCallback(async (selectedGender: 'male' | 'female') => {
    await analytics.trackGenderSelected(licenseKey, selectedGender);
  }, [licenseKey]);

  // Track measurement guide opened
  const trackMeasurementGuideOpened = useCallback(async (measurementType: 'chest' | 'waist' | 'hips') => {
    await analytics.trackMeasurementGuideOpened(licenseKey, measurementType);
  }, [licenseKey]);

  // Track size recommendation
  const trackSizeRecommendation = useCallback(async (recommendedSize: string) => {
    if (gender && !hasTrackedRecommendation) {
      await analytics.trackSizeRecommended(licenseKey, gender, recommendedSize);
  ...
  const trackAddToCart = useCallback(async (size: string) => {
    console.log('🔍 trackAddToCart called in popup:', { productId, licenseKey, size });
    if (productId) {
  ...
  useEffect(() => {
    ...
    // Track completion of previous step if it exists
    if (previousStep > 0 && previousStep !== currentStep) {
      ...
      const stepId = `step_${previousStep}`;
      const stepName = stepNames[previousStep as keyof typeof stepNames] || `step_${previousStep}`;
      
      trackStepCompletion(stepId, stepName);
    }
    
    // Start tracking new step
    if (currentStep !== previousStep) {
      const stepId = `step_${currentStep}`;
      startStepTracking(stepId);
    }
    
    prevStepRef.current = currentStep;
  }, [step, trackStepCompletion, startStepTracking]);
  ...
  const getApiSizeRecommendation = async () => {
    ...
    const requestData = {
      ...
      use_ai: true,
      brand_id: brandId || "default",  // Use prop or fallback to default
      product_id: productId || "default",  // Use prop or fallback to default
  ...
  const renderMeasurementInputs = (type: 'height' | 'weight' | 'chest' | 'waist' | 'hips') => {
    ...
    const handleSliderChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Track user edits for chest, waist, hips
  ...
  // Trigger API call when reaching final step or when data changes on final step
  useEffect(() => {
    ...
    // Clear API result when navigating away from final step
    if (step !== finalStep && apiSizeResult) {
      console.log('🔄 Clearing API result - navigating away from final step');
      setApiSizeResult(null);
      dataHashRef.current = '';
    }
    
    // Check if we're on the final step and should make an API call
    if (step === finalStep && useApiRecommendation && !isApiLoading) {
      ...
      // Store the hash in a ref to compare with previous values
      ...
      // 2. Data has changed since last API call
      const shouldMakeApiCall = !apiSizeResult || dataHashRef.current !== currentDataHash;
      
      if (shouldMakeApiCall) {
        console.log('🚀 Triggering API call for size recommendation...', {
          reason: !apiSizeResult ? 'No existing result' : 'Data changed',
        ...
        getApiSizeRecommendation().then(result => {
          if (result) {
            ...
            // Track size recommendation
            trackSizeRecommendation(result.size);
            // Don't call onSizeRecommended here to prevent auto-closing
  ...
  }, [step, isMobile, useApiRecommendation, apiSizeResult, isApiLoading, gender, measurements, fitPreference, trackSizeRecommendation]);

  // Set default measurements when gender changes
  useEffect(() => {
    if (gender === 'male') {
      setMeasurements({
        height: "170",
  ...
  useEffect(() => {
    ...
    // Clear API result when measurements change to force fresh API call
    if (apiSizeResult) {
      console.log('🔄 Clearing API result - measurements changed');
      setApiSizeResult(null);
      dataHashRef.current = '';
    }
  }, [measurements.height, measurements.weight]);

  // Clear API result when body measurements or fit preference change
  useEffect(() => {
    if (apiSizeResult) {
      console.log('🔄 Clearing API result - body measurements or fit preference changed');
      setApiSizeResult(null);
      dataHashRef.current = '';
    }
  }, [measurements.chest, measurements.waist, measurements.hips, fitPreference]);
                    ...
                    {step > 1 && (
                      <button
                        onClick={async () => {
                          ...
                          const stepName = stepNames[step as keyof typeof stepNames] || `step_${step}`;
                          await trackStepCompletion(stepId, stepName);
                        }}
