...
interface RadialChestSliderProps {
  value: number; // chest in cm
...
const MIN_CM = 70;  // ~28 inches
const MAX_CM = 150; // ~59 inches
...
export const RadialChestSlider: React.FC<RadialChestSliderProps> = ({ value, onChange, useMetric }) => {
  ...
  const { ticksMetric, ticksImperial, paddedTicks } = useMemo(() => {
    ...
    for (let c = MIN_CM; c <= MAX_CM; c += 1) {
      const isMajor = c % 5 === 0; // Highlight every 5cm marks
      const isLabel = c % 10 === 0; // Show labels every 10cm
      metric.push({
        cm: c,
    ...
    for (let i = MIN_INCHES; i <= MAX_INCHES; i += 1) {
      const isMajor = i % 5 === 0; // Highlight 5 inch marks
  ...
  useEffect(() => {
    if (useMetric) {
      ...
      if (rulerRef.current) {
        ...
        const cmIndex = (targetCm - MIN_CM) / 1; // 0-based index in the metric array
  ...
  const applyMomentum = useCallback(() => {
    ...
    const momentumDistance = velocity.current * 0.8; // Increased momentum multiplier for smoother feel
    ...
    const duration = Math.min(1500, Math.max(600, Math.abs(velocity.current) * 200)); // Increased duration for smoother animation
  ...
  const onDragMove = useCallback((e: React.TouchEvent | React.MouseEvent) => {
    ...
    if (lastTime.current > 0) {
      ...
      if (deltaTime > 0) {
        ...
        velocity.current = (velocity.current * 0.7) + ((deltaX / deltaTime) * 20 * 0.3);
      }
    }
