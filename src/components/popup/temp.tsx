...
export function SizingAssistant({ onClose, onSizeRecommended }: SizingAssistantProps) {
  ...
  useEffect(() => {
    if (gender === 'male') {
      const bodyMeasurements = calculateBodyMeasurements(170, 75, 'male');
      setMeasurements({
        height: "170",
        weight: "75",
        chest: bodyMeasurements.chest.toString(),
        waist: bodyMeasurements.waist.toString(),
        hips: bodyMeasurements.hips.toString()
      });
      
      // Console log for verification
      console.log('Initial BMI Calculation (Male):', { height: 170, weight: 75, gender: 'male' });
      console.log('Initial BMI Output (Male):', bodyMeasurements);
    } else if (gender === 'female') {
      const bodyMeasurements = calculateBodyMeasurements(160, 60, 'female');
      setMeasurements({
        height: "160",
        weight: "60",
        chest: bodyMeasurements.chest.toString(),
      ...
      // Console log for verification
      console.log('Initial BMI Calculation (Female):', { height: 160, weight: 60, gender: 'female' });
      console.log('Initial BMI Output (Female):', bodyMeasurements);
    }
  }, [gender])
  ...
  const handleAddToBag = () => {
    ...
    // Call the callback with the recommended size
    onSizeRecommended(size);
  ...
  // Function to update measurements based on BMI calculation
  const updateMeasurementsFromBMI = (height: number, weight: number) => {
    if (!gender || height <= 0 || weight <= 0) return;

    try {
      const bodyMeasurements = calculateBodyMeasurements(height, weight, gender);
      
      // Console log for verification
      console.log('BMI Calculation Input:', { height, weight, gender });
      console.log('BMI Calculation Output:', bodyMeasurements);
      ...
      console.error('Error calculating BMI measurements:', error);
    }
  };
  ...
  const renderMeasurementInputs = (type: 'height' | 'weight' | 'chest' | 'waist' | 'hips') => {
    ...
    const handleSliderChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Update BMI-based measurements when height or weight changes
      if (type === 'height' || type === 'weight') {
        const currentHeight = type === 'height' ? metricValue : parseFloat(measurements.height);
        const currentWeight = type === 'weight' ? metricValue : parseFloat(measurements.weight);
        updateMeasurementsFromBMI(currentHeight, currentWeight);
      }
    };
        ...
        <div className="space-y-2 md:space-y-3">
          ...
          <RadialWeightSlider
            ...
            onChange={(newValue) => {
              ...
              // Update BMI-based measurements when weight changes
              const currentHeight = parseFloat(measurements.height);
              updateMeasurementsFromBMI(currentHeight, newValue);
            }}
            useMetric={useMetric}
          />
        </div>
  ...
  }, [step, gender, measurements, isMobile, fitPreference, hasTrackedRecommendation]);
  ...
  const renderStepContent = () => {
                ...
                <>
                  ...
                  <RadialHeightSlider
                    ...
                    onChange={val => {
                      ...
                      // Update BMI-based measurements when height changes
                      const currentWeight = parseFloat(measurements.weight);
                      updateMeasurementsFromBMI(val, currentWeight);
                    }}
                    useMetric={useMetric}
                  />
    ...
    switch (step) {
            ...
            {isMobile ? (
              <RadialWeightSlider
                ...
                onChange={val => {
                  ...
                  // Update BMI-based measurements when weight changes
                  const currentHeight = parseFloat(measurements.height);
                  updateMeasurementsFromBMI(currentHeight, val);
                }}
                useMetric={useMetric}
              />
            ) : (
      ...
      case 8:
        const { size: mobileSize, confidence: mobileConfidence } = calculateSize()
        return (
          <div className="space-y-4">
