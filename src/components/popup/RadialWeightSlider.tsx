...
export const RadialWeightSlider: React.FC<RadialWeightSliderProps> = ({ value, onChange, useMetric }) => {
  ...
  const { ticksMetric, ticksImperial, paddedTicks } = useMemo(() => {
    ...
    for (let k = MIN_KG; k <= MAX_KG; k += 1) {
      ...
      const isLabel = k % 10 === 0; // Show labels every 10kg
      metric.push({
        kg: k,
    ...
    for (let l = roundedMinLbs; l <= roundedMaxLbs; l += 1) {
      ...
      const isLabel = l % 10 === 0; // Show labels every 10lbs
      imperial.push({
        lbs: l,
  ...
  useEffect(() => {
    if (useMetric) {
      ...
      if (rulerRef.current) {
        ...
        const kgIndex = (targetKg - MIN_KG) / 5; // 0-based index in the metric array
      ...
      if (rulerRef.current) {
        ...
        const lbsIndex = (lbsValue - MIN_LBS) / 10; // 0-based index in imperial array
